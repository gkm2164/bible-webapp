    ...
    $.get("/bibles", (data) => {
        ...
        $("nav").append(
            $("<ul></ul>").append(
                books.map((b, idx) => {
                    const id = idx + 1;
                    return `<li id="menu-${id}" data-verses="${b.chapters.length}"><a href="#book-${id}">${b.name}</a>`;
                })
            )
        );
        ...
        $("section").append(
            books.map((book, bookId) => {
                ...
                return $("<div></div>").append(`<h2><a id="book-${bookId + 1}">${name}</a></h2>`).append(
                    ...chapters.flatMap(chapter =>
                        [$(`<h3 class="chap-header"><a id="chap-${bookId}-${chapter.id}">${chapter.id}</a></h3>`),
